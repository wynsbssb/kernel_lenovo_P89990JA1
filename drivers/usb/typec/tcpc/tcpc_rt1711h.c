...
#define ET_7303_VID	0x6dcf
#define ET_7303_PID	0x1711
#define ET_7303_DID	0x2171
...
static int rt1711_regmap_init(struct rt1711_chip *chip)
{
	...
	if ((chip->chip_id == HUSB311_DID) && (chip->chip_pid == HUSB311_PID)) {
...
//OAK-790,liangsuocheng.wt modified 20211218 fix otg fail
#if 0 
//#ifdef CONFIG_TCPC_I2CRST_EN
	...
	}else if ((chip->chip_id == SC2150A_DID) && (chip->chip_pid != HUSB311_PID)) {
...
static int rt1711_get_cc(struct tcpc_device *tcpc, int *cc1, int *cc2)
{
	...
	*cc2 = TCPC_V10_REG_CC_STATUS_CC2(status);
	...
	if((chip->chip_id == SC2150A_DID) && (chip->chip_pid != HUSB311_PID) && act_as_drp &&
			act_as_sink) {
...
static int rt1711_set_cc(struct tcpc_device *tcpc, int pull)
{
	...
	if ((chip->chip_id == SC2150A_DID) && (chip->chip_pid != HUSB311_PID))
	...
	if (pull == TYPEC_CC_DRP) {
		...
		if (chip->chip_id!= SC2150A_DID || chip->chip_pid == HUSB311_PID || old_data != data) {
...
static int rt_parse_dt(struct rt1711_chip *chip, struct device *dev)
{
	...
	if (ret < 0)
		pr_err("%s no intr_gpio info\n", __func__);
#endif
	ret = of_get_named_gpio(np, "rt1711h-en", 0);
	if (ret) {
		pr_err("lsw %s no rt1711h-en info\n", __func__);
		return ret;
	}
	chip->ldo_en_gpio = ret;
	gpio_direction_output(chip->ldo_en_gpio, true);
...
static int rt1711_tcpcdev_init(struct rt1711_chip *chip, struct device *dev)
{
	...
	if ((chip->chip_id > RT1711H_DID_B) || ((chip->chip_id == HUSB311_DID) && (chip->chip_pid == HUSB311_PID)))
...
static inline int rt1711h_check_revision(struct i2c_client *client)
{
	...
	if ((vid != RICHTEK_1711_VID) && (vid != ET_7303_VID) && (vid != SC2150A_VID) && (vid != HUSB311_VID)) { /*&& (vid != HYNETEK_HUSB311_VID) */ 
	...
	if ((pid != RICHTEK_1711_PID) && (pid != ET_7303_PID) && (pid != SC2150A_PID) && (pid != HUSB311_PID)) { /*&& (pid != HYNETEK_HUSB311_PID) */
	...
	if ((did != RICHTEK_1711_DID) && (did != ET_7303_DID) && (did != SC2150A_DID)) {
		pr_info("%s failed, DID=0x%04x\n", __func__, did);
		return -ENODEV;
	}
				...
				const struct i2c_device_id *id)
{
	...
	chip->chip_pid = (int)chip_pid;
	pr_info("rt1711h_chipID = 0x%0x\n", chip_id);
	pr_info("rt1711h pid = 0x%0x\n", chip_pid);
